...
- [ ] 4.4 両面印刷対応の名札生成機能の実装

  - 表面・裏面それぞれの Canvas 生成機能を実装
  - 両面テンプレートシステムの構築
  - 表面・裏面の独立した枠サイズ設定機能を追加
  - _要件: 5.1, 5.3, 5.4_

- [ ] 4.5 A4 サイズ両面レイアウトシステムの実装

  - A4 サイズ（297mm × 210mm）の横向きレイアウト設定を実装
  - 左半分（表面）と右半分（裏面）の配置システムを構築
  - cm 単位から mm 単位への変換機能を実装
  - _要件: 5.2, 5.3, 5.4_

- [ ] 4.6 枠サイズ設定 UI コンポーネントの実装

  - 表面・裏面それぞれの cm 単位入力フィールドを作成
  - リアルタイムプレビュー更新機能を実装
  - 枠サイズバリデーション機能を追加（最小・最大値チェック）
  - _要件: 5.4, 5.5, 5.7_
...
- [x] 7. Chrome 拡張機能 UI の実装
- [x] 7.1 ポップアップ UI の基本実装
...
- [x] 8. バックグラウンドスクリプトの実装
- [x] 8.1 拡張機能の初期化とライフサイクル管理
